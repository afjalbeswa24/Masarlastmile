...
class _BulkUploadScreenState extends State<BulkUploadScreen> {
  ...
  void _processRows(List<List<dynamic>> rows) {
    ...
    for (final col in _requiredColumns) {
      if (!headers.contains(col)) {
        setState(() => _errors = ['Missing required column: "$col". Check your file headers match exactly.']);
        return;
      }
    }
  ...
  Future<void> _uploadAll() async {
    ...
    try {
      ...
      final rowsWithMerchant = _validRows.map((r) => {...r, 'merchant_id': merchantId}).toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Bulk Upload Orders')),
      body: Padding(
          ...
          children: [
            Card(
              ...
              child: const Padding(
                ...
                child: Text(
                  'Required columns: Consignee Name, Phone, Full Address.\n'
                  'Optional: City, Quantity, COD Amount, Delivery Date (YYYY-MM-DD), '
                  'After (HH:MM), Before (HH:MM), Notes.',
                ...
                child: ListView(
                  children: [
                    ...
                    if (_errors.isNotEmpty) ...[
                      ...
                      ..._errors.map((e) => Text('• $e', style: const TextStyle(color: Colors.red, fontSize: 12))),
                    ],
                  ],
                ),
              ),
